...
export class ChatAddCommand extends BaseCommand {
  ...
  public async execute(interaction: ChatInputCommandInteraction) {
    ...
    try {
      ...
      const masterchat = await this.youtubeService.add(id, channelId)
      if (!masterchat) {
      ...
      await interaction.editReply({
        embeds: [{
          ...
          description: `**[${masterchat.videoId}](${YoutubeUtils.getVideoUrl(masterchat.videoId)})** chat relay added to <#${channelId}>`,
          color: 0x1d9bf0,
          author: {
